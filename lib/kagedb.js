(function (env) {
    ...
    function KageDB(settings) {
        ...
        this._onerror_internal = function (event) {
            event.kage_errorMessage = KageDB.prefix("An error occurred. " + makeMessage(event));
            if (self.onerror) self.onerror.call(null, event);
        };
        this._onblocked_internal = function (event) {
            event.kage_errorMessage = KageDB.prefix("A blocking occurred. " + makeMessage(event));
        ...
        function makeMessage(event) {
            ...
            return JSON.stringify({
                ...
                detail: KageDB.errorMessages[target.errorCode]
            });
        }
    }
    ...
    KageDB.prototype.tx = function tx(storeNames, mode, success, error) {
        ...
        this._registerListeners(req, null, this._onerror_internal);

        if (this._onerror_internal) {
            req.onerror = this._onerror_internal.bind(null);
        }

        if (this._onblocked_internal) {
            req.onblocked = this._onblocked_internal.bind(null);
        }
        ...
        req.onsuccess = function (event) {
            ...
            function do_onsuccess() {
                ...
                if (error) transaction.onerror = error.bind(null);
