(function (env) {
    ...
    var helper = {
        ...
        fetch: function fetch(instance, source, range, direction, success, error) {
            var keyOnly = range && range.keyOnly;
            ...
            helper.attachInfo(req, instance, "fetch", { range: range, direction: direction });

            var filter;
            if (range && typeof range.filter === "function") {
                filter = range.filter;
            }

            var reduce;
            if (range && typeof range.reduce === "function") {
                var original_reduce = range.reduce;
                var prev = range.initialValue;
                var called;
            ...
            var limit;
            if (range && typeof range.limit === "number") {
                limit = range.limit;
            }
    ...
    KageObjectStore.prototype.fetch = function fetch(range, direction, success, error) {
        ...
        return helper.fetch(this, this._objectStore, range, direction, success, error);
    };
    ...
    KageIndex.prototype.fetch = function fetch(range, direction, success, error) {
        ...
        return helper.fetch(this, this._index, range, direction, success, error);
    };
